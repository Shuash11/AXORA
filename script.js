(() => {
  ...
  function createParticles() {
    if (!particleContainer || prefersReducedMotion || particleContainer.childElementCount > 0) {
      return;
    }
    ...
    for (let index = 0; index < 12; index += 1) {
      ...
      particle.style.inlineSize = `${size.toFixed(1)}px`;
      particle.style.blockSize = `${size.toFixed(1)}px`;
      particle.style.left = `${(Math.random() * 100).toFixed(1)}vw`;
