...
test('person-stage is a transparent preserve-3d control carrying tilt and lift', () => {
  ...
  assert.match(css, /\.person-stage\s*\{[^}]*transition:\s*transform[^;]+,[^}]*opacity/, 'the stage must transition only transform and opacity');
});
...
test('hover, focus, and selection lift the stage and tighten the floor shadow', () => {
  assert.match(css, /\.person-stage:not\(:disabled\):hover[\s\S]*?--lift:\s*-7px/, 'hover must lift the stage -7px');
  assert.match(css, /\.person-stage:not\(:disabled\):focus-visible[\s\S]*?--lift:\s*-7px/, 'keyboard focus must lift the stage -7px');
